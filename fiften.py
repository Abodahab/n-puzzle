        ...
        while queue:
            ...
            for move in end_node.get_moves():
                if move.position in expanded:
                    continue
                new_path = [
                    path[0] + self._calculate_new_heuristic(move, end_node)] + path[1:] + [move]
...
class Puzzle:
    ...
    def _get_inversions_count(self):
        inv_count = 0
        puzzle_list = [
            number for row in self.position for number in row if number != 0]
    ...
    def heuristic_manhattan_distance(self):
        ...
        for i in range(self.PUZZLE_NUM_ROWS):
            for j in range(self.PUZZLE_NUM_COLUMNS):
                i1, j1 = self._get_coordinates(
                    self.position[i][j], self.PUZZLE_END_POSITION)
...
#    puzzle = Puzzle([[1, 2, 3, 4], [5, 6, 7, 8], [
#                    0, 10, 11, 12], [9, 13, 14, 15]])
